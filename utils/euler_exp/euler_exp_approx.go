...
func ApproximateExp(K, degree int, dev float64, scnum int) (cosin, sine []*big.Float) {
	...
	nodes_cosine, y := genNodes(deg, dev, totdeg, K, scnum, true)
	nodes_sine, y := genNodes(deg, dev, totdeg, K, scnum, false)

	cosine_poly := solve(totdeg, K, scnum, nodes_cosine, y)[:totdeg] //minimax cosin function
	sine_poly := solve(totdeg, K, scnum, nodes_sine, y)[:totdeg] //minimax sine function
	// Solves the linear system and returns the coefficients
	return cosine_poly, sine_poly
}

// y = cos( (2 * pi * x)/r )
func cos2PiXOverR(x, r *big.Float) (y *big.Float) {
	//y = 2 * pi
...
// y = cos( (2 * pi * x)/r )
func sin2PiXOverR(x, r *big.Float) (y *big.Float) {
	//y = 2 * pi
	y = bignum.NewFloat(2.0, EncodingPrecision)
	y.Mul(y, pi)
	// x = x / r
...
func genNodes(deg []int, dev float64, totdeg, K, scnum int, isCos bool) ([]*big.Float, []*big.Float) {
	...
	for i := 0; i < totdeg; i++ {
		// y[i] = cos(2*pi*(nodes[i]/r) or sin(2*pi*(nodes[i]/r)
		if isCos {
			y[i] = cos2PiXOverR(nodes[i], scfac)
		} else{
			y[i] = sin2PiXOverR(nodes[i], scfac)
		}
	}
